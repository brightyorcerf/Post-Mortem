...
What makes Shadow Register unique among cybersecurity environments is its embedded **Ground Truth DAG** — a Directed Acyclic Graph that encodes the precise Kill Chain the attacker executed. Each node in the DAG represents a verifiable forensic fact (an IP address, a timestamp, a malicious file path) with a weighted contribution to the final score. The DAG's edges encode **causal prerequisite relationships**: an agent that identifies the C2 callback IP without first locating the persistence mechanism receives reduced credit due to a broken chain.
...
class ForensicAction(BaseModel):
    action: ActionType           # Search | Inspect | Read | Tag | SubmitCase
    query:  Optional[str]        # Search: keyword
    path:   Optional[str]        # Read/Inspect: absolute file path
    offset: Optional[int] = 0    # Read: byte offset for chunked reading
    label:  Optional[str]        # Tag: evidence key
    value:  Optional[str]        # Tag: evidence value
    pivots: Optional[List[ForensicPivot]]  # SubmitCase: final report
```
...
#### ForensicPivot Structure

```python
class ForensicPivot(BaseModel):
    artifact: str     # Path to the source file (e.g. "/var/log/auth.log")
    ioc:      str     # The extracted IOC value (e.g. "185.47.92.13")
    type:     IOCType # NETWORK_IP | EVENT_TIMESTAMP | PATH_TO_FILE |
                      # COMMAND_STRING | USER_ACCOUNT | FILE_HASH
    reason:   str     # Causal justification for the pivot
```

### 2.2 · Observation Space: `ForensicObs` (Pydantic Model)
...
Shadow Register ships three investigation scenarios with clear difficulty progression. All three are procedurally generated from a seed, graded against a deterministic TruthDAG, and scored from **0.0 to 1.0**.

---

### 3.1 · `noisy_entry` — The Noisy Entry (Easy)
...
### 3.2 · `stealthy_persistence` — The Stealthy Persistence (Medium)
...
```bash
# Verify σ=0 determinism across 100 iterations
python tests/evaluateStability.py
```
...
Baseline performance of frontier models on all three tasks (seed=42, single run):

| Model | `noisy_entry` (Easy) | `stealthy_persistence` (Medium) | `timestomp_proxy` (Hard) |
|:------|:--------------------:|:-------------------------------:|:------------------------:|
| **gpt-4o** | 0.92 | 0.65 | 0.25 |
| **gpt-4o-mini** | 0.85 | 0.40 | 0.10 |
| **Oracle (upper bound)** | 1.00 | 1.00 | 1.00 |
| **Honeypot-only (trap)** | 0.00 | 0.00 | 0.00 |

**Observations:**
- The Easy task is solvable by most capable models — the brute-force cluster is loud and localised.
- The Medium task separates models that can perform multi-hop reasoning (crontab → launcher → base64 decode → C2 extraction) from those that stop at surface-level grep.
- The Hard task is where current frontier models struggle: metadata reasoning (mtime vs. ctime discrepancy), cross-artifact correlation (binary size vs. dpkg record), and dual-decoy resistance require capabilities that go beyond text comprehension.

---

## 7 · Creativity & Novelty

### 7.1 · The Honeypot Mechanic
...
### 7.2 · Procedural World Generation
...
| File | Lines | Role |
|:-----|------:|:-----|
| `schema.py` | 105 | Pydantic data models: `ForensicAction`, `ForensicObs`, `TruthDAG`, `IOCType`, etc. |
| `worldGen.py` | 907 | Procedural world generator: 3 scenario builders + noise layer + deterministic RNG |
| `env.py` | 422 | OpenEnv game loop: `reset()`, `step()`, `state()`, budget management, milestone tracking |
| `grader.py` | 327 | Deterministic scoring: node matching, chain validation, honeypot penalties, efficiency bonus |
| `server.py` | 203 | FastAPI HTTP wrapper: 4 endpoints, session management, JSON serialisation |
| `inference.py` | 389 | Baseline agent: OpenAI-compatible LLM client, structured logging, CLI |
| `openenv.yaml` | 198 | Full OpenEnv specification: tasks, observation/action schemas, reward components |
| `Dockerfile` | 42 | Production container: Python 3.11-slim, non-root user, health check |
...
**Shadow Register** was built because we believe forensic reasoning is one of the hardest — and most consequential — capabilities an autonomous agent can develop. It's not about finding a flag. It's about understanding *why* a particular IP appears at a particular time in a particular log, and whether that log is telling the truth.

*The attacker has already left. The evidence is all that remains.*
*Can your agent read the post-mortem?*

---

**License:** MIT · **OpenEnv v1.0 Compliant** · **σ=0 Deterministic**
